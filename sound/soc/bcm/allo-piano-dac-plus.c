		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = (~(left_val  >> mc->shift)) & mc->max;
	ucontrol->value.integer.value[1] = (~(right_val >> mc->shift)) & mc->max;
		...
		struct snd_pcm_hw_params *params)
{
	...
	//dev_warn(card->dev, "CEM hw_param before snd_soc_dai_set_bclk_ratio :%d \n",sample_bits);
	ret = snd_soc_dai_set_bclk_ratio(cpu_dai, sample_bits * 2);
	//dev_warn(card->dev, "CEM 1hw_param before snd_soc_dai_set_bclk_ratio \n");
